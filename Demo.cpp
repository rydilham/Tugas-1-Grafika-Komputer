...
void Demo::Update(double deltaTime) {
	...
	speed += (float)((deltaTime * 1.5f) / 1000);

	if (altitude >= 10 || altitude < 0) {
		speedy *= -1;
	}

	altitude += (float)((deltaTime * speedy) / 100);
	velocity = path();
}

glm::vec3 path() {
	glm::vec3 direction;
	glm::vec3 route[3] = { glm::vec3 (0.0f,2.0f,0.0f),
...
void Demo::Render() {
	...
	DrawColoredCube(10, 0.5, 1, 0, 4.25, -20, 1);//propeller 1
	DrawColoredCube(5, 3, 3, 0, 2.5, -20, 0);//body
	DrawColoredCube(6, 1, 1, 5.5, 2.5, -20, 0);//tail
	DrawColoredCube(0.5, 3.5, 0.5, 8, 2.5, -19.5, 2);//propeller2
...
void Demo::DrawColoredCube(float s1, float s2, float s3, float l1, float l2, float l3, int a)
{
	...
	glm::mat4 model;
	model = glm::translate(model, glm::vec3(l1 + 15, l2 , l3) + glm::vec3(10,10,10));
